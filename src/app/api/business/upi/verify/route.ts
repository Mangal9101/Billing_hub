...
import { supabaseAuthUserWithRefresh, verifyCompanyMembership, verifyBusinessOwner } from '@/lib/server-supabase';
...
async function razorpayFetch(path: string, init: RequestInit = {}) {
  ...
  if (!header) throw new Error('UPI bank-name verification is not configured. Add the RazorpayX verification credentials in Vercel.');
...
export async function POST(req: NextRequest) {
  try {
    ...
    const role = String(auth.role || membership.role || '').toLowerCase();
    ...
    if (!sourceAccountNumber) {
      return NextResponse.json({ error: 'UPI bank-name verification is not configured yet. Add RAZORPAYX_SOURCE_ACCOUNT_NUMBER in Vercel.' }, { status: 500 });
    }
    ...
    const createResponse = await razorpayFetch('/v1/fund_accounts/validations', {
      ...
      body: JSON.stringify({
        ...
        notes: { source: 'billing_hub', business_id: businessId, upi_id: upiId },
        fund_account: {
          account_type: 'vpa',
          vpa: { address: upiId },
          contact: {
    ...
    if (!createResponse.ok) {
      ...
      return NextResponse.json({ error: detail || 'Razorpay could not start UPI verification.' }, { status: createResponse.status >= 400 && createResponse.status < 500 ? createResponse.status : 502 });
    }

    const validationId = String(created?.id || '');
    if (!validationId) return NextResponse.json({ error: 'Razorpay did not return a verification ID.' }, { status: 502 });

    let latest = created;
    for (let i = 0; i < 8; i += 1) {
      const status = String(latest?.status || '').toLowerCase();
      const registeredName = String(latest?.validation_results?.registered_name || '').trim();
      if (status === 'completed' || registeredName || status === 'failed') break;
    ...
    if (status === 'failed' || accountStatus === 'invalid' || !registeredName) {
      const detail = String(latest?.validation_results?.details || latest?.status_details?.description || latest?.status_details?.reason || '');
      return NextResponse.json({ error: detail || 'This UPI ID could not be verified or its bank-registered name was not returned.' }, { status: 400 });
    }

    return NextResponse.json({ ok: true, verified: true, upiId, registeredName, accountStatus: accountStatus || 'active', validationId });
  } catch (e: any) {
    console.error('UPI BANK NAME VERIFY ERROR:', e);
    return NextResponse.json({ error: e?.message || 'Unable to verify UPI ID.' }, { status: 500 });
  }
}
