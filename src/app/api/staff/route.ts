...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ staff: Array.isArray(staff) ? staff.map((s:any) => ({ ...s, permissions: sanitizeStaffPermissions(String(s.role || 'cashier').toLowerCase() as any, s.permissions) })) : [] });
